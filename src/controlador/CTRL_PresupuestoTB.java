...
public class CTRL_PresupuestoTB {
    ...
    public void EnEscucha() {
        ...
        v_presupuesto.jb_nuevo.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                ArrayList<OBJ_Reglones> seleccionados = extraerDatosTabla();
                v_presupuesto.jl_total.setText("$ " + calcularTotales(seleccionados));
                new CTRL_DatosPersonales().setPresupuestoGenerado(seleccionados);
                v_presupuesto.dispose();
            }
        });

        v_presupuesto.jb_consulta.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                ArrayList<OBJ_Reglones> seleccionados = new ArrayList<>();
                seleccionados.add(new OBJ_Reglones("Mostrador", "Consulta", 1, 200));
                v_presupuesto.jl_total.setText("$ " + calcularTotales(seleccionados));
                new CTRL_Cobrar(seleccionados);
                v_presupuesto.dispose();
            }
        });
    }
